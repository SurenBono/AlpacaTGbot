...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .setting-row label { width: 45%; font-size: 0.9em; }
...
<body>
    <div class="container">
        ...
        <div class="grid">
            <!-- Account Status Card with P&L Details -->
            <div class="card">
                ...
                <div class="pnl-row">
                    ...
                    <span class="{{ 'positive' if account.today_pnl >= 0 else 'negative' }}">${{ "%.2f"|format(account.today_pnl) }}</span>
                </div>
                <div class="pnl-row">
                    <span>📊 Total Realized P&L:</span>
                    <span class="{{ 'positive' if account.total_realized_pnl >= 0 else 'negative' }}">${{ "%.2f"|format(account.total_realized_pnl) }}</span>
                </div>
                <div class="pnl-row">
                    <span>💹 Unrealized P&L:</span>
                    <span class="{{ 'positive' if account.unrealized_pnl >= 0 else 'negative' }}">${{ "%.2f"|format(account.unrealized_pnl) }}</span>
                </div>
                <div class="pnl-row">
                    <span>🎯 Total P&L (Realized + Unrealized):</span>
                    <span class="{{ 'positive' if account.total_pnl >= 0 else 'negative' }}">${{ "%.2f"|format(account.total_pnl) }}</span>
                </div>
                <div class="pnl-row">
                    <span>📉 Today's P&L %:</span>
                    <span class="{{ 'positive' if account.today_pnl_percent >= 0 else 'negative' }}">{{ "%.2f"|format(account.today_pnl_percent) }}%</span>
                </div>
            ...
            <div class="card">
                ...
                <div class="setting-row">
                    ...
                    <input type="number" id="min_profit_usd" step="0.1" value="{{ current_settings.min_profit_usd }}" style="width: 50%;" onchange="updateSetting('min_profit_usd', parseFloat(this.value))">
                </div>
            ...
            <!-- Active Positions Card with Close All Button -->
            <div class="card">
                <h3>📈 Active Positions 
                ...
                {% if not positions %}
                <div style="text-align: center; padding: 20px; color: #888;">No active positions</div>
                {% endif %}
            </div>
        </div>
    ...
    <script>
        let botStartTime = {{ bot_start_time }};
            ...
            fetch('/api/status')
                ...
                .then(data => {
                    ...
                    if (data.current_settings) {
                        ...
                        if (sens && parseFloat(data.current_settings.sensitivity) !== parseFloat(sens.value)) {
                        ...
                        if (atr && parseInt(data.current_settings.atr_period) !== parseInt(atr.value)) {
                        ...
                        if (sl && parseFloat(data.current_settings.stop_loss_percent * 100) !== parseFloat(sl.value)) {
                        ...
                        if (mp && parseFloat(data.current_settings.min_profit_usd) !== parseFloat(mp.value)) {
...
class CryptoTradingBot:
    ...
    def add_log(self, message, level="INFO"):
        ...
        # Keep only last 30 logs in memory
    ...
    def get_account_status(self):
        try:
            ...
            today_pnl_percent = (self.today_realized_pnl / float(account.equity) * 100) if float(account.equity) > 0 else 0
...
def main():
    ...
    try:
        ...
        print("   P&L details now shown in Account Status card")
        print("")
