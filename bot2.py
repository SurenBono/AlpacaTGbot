...
# HTML Template for Dashboard with Live Controls
DASHBOARD_TEMPLATE = """
<!DOCTYPE html>
<html>
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .setting-row input[type="range"] { width: 55%; }
        .value-display { 
            font-size: 0.9em; 
            color: #00d4ff;
            margin-left: 10px;
...
<body>
            ...
            <div class="card">
                ...
                <div class="setting-row">
                    ...
                    <select id="timeframe" onchange="updateSetting('timeframe', this.value)">
                        <option value="1Min" {{ 'selected' if current_settings.timeframe == '1Min' else '' }}>1 Minute</option>
                        <option value="5Min" {{ 'selected' if current_settings.timeframe == '5Min' else '' }}>5 Minutes</option>
                        <option value="15Min" {{ 'selected' if current_settings.timeframe == '15Min' else '' }}>15 Minutes</option>
                        <option value="30Min" {{ 'selected' if current_settings.timeframe == '30Min' else '' }}>30 Minutes</option>
                        <option value="1H" {{ 'selected' if current_settings.timeframe == '1H' else '' }}>1 Hour</option>
                        <option value="2H" {{ 'selected' if current_settings.timeframe == '2H' else '' }}>2 Hours</option>
                        <option value="4H" {{ 'selected' if current_settings.timeframe == '4H' else '' }}>4 Hours</option>
                        <option value="1D" {{ 'selected' if current_settings.timeframe == '1D' else '' }}>1 Day</option>
                    </select>
                </div>
                
                <div class="setting-row">
                    <label>🎯 Sensitivity (a):</label>
                    <input type="range" id="sensitivity" min="0.5" max="2.0" step="0.05" value="{{ current_settings.sensitivity }}" oninput="updateSensitivityValue(this.value)" onchange="updateSetting('sensitivity', parseFloat(this.value))">
                    <span id="sensitivityValue" class="value-display">{{ "%.2f"|format(current_settings.sensitivity) }}</span>
                </div>
                
                <div class="setting-row">
                    <label>📊 ATR Period:</label>
                    <input type="range" id="atr_period" min="5" max="20" step="1" value="{{ current_settings.atr_period }}" oninput="updateATRValue(this.value)" onchange="updateSetting('atr_period', parseInt(this.value))">
                    <span id="atrValue" class="value-display">{{ current_settings.atr_period }}</span>
                </div>
                
                <div class="setting-row">
                    <label>🛡️ Stop Loss %:</label>
                    <input type="range" id="stop_loss" min="1" max="10" step="0.5" value="{{ current_settings.stop_loss_percent * 100 }}" oninput="updateStopLossValue(this.value)" onchange="updateSetting('stop_loss', parseFloat(this.value) / 100)">
                    <span id="stopLossValue" class="value-display">{{ "%.1f"|format(current_settings.stop_loss_percent * 100) }}%</span>
                </div>
                
                <div class="setting-row">
                    <label>💰 Min Profit $:</label>
                    <input type="number" id="min_profit_usd" step="0.1" value="{{ current_settings.min_profit_usd }}" style="width: 55%;" onchange="updateSetting('min_profit_usd', parseFloat(this.value))">
                </div>
                
                <div class="setting-row">
                    <label>📈 Heikin Ashi:</label>
                    <select id="heikin_ashi" onchange="updateSetting('heikin_ashi', this.value === 'true')">
                        <option value="true" {{ 'selected' if current_settings.heikin_ashi else '' }}>Enabled</option>
                        <option value="false" {{ 'selected' if not current_settings.heikin_ashi else '' }}>Disabled</option>
                    </select>
                </div>
                ...
                <div class="setting-row">
                    <button onclick="fetch('/close_all_positions', {method: 'POST'})" class="danger">Close All Positions</button>
    ...
    <script>
        function updateSensitivityValue(val) {
            document.getElementById('sensitivityValue').innerText = parseFloat(val).toFixed(2);
        }
            ...
            fetch('/api/status')
                ...
                .then(data => {
                    ...
                    if (data.current_settings) {
                        if (data.current_settings.timeframe !== document.getElementById('timeframe').value) {
                            document.getElementById('timeframe').value = data.current_settings.timeframe;
                        }
                        if (parseFloat(data.current_settings.sensitivity) !== parseFloat(document.getElementById('sensitivity').value)) {
                            document.getElementById('sensitivity').value = data.current_settings.sensitivity;
                            document.getElementById('sensitivityValue').innerText = parseFloat(data.current_settings.sensitivity).toFixed(2);
                        }
                        if (parseInt(data.current_settings.atr_period) !== parseInt(document.getElementById('atr_period').value)) {
                            document.getElementById('atr_period').value = data.current_settings.atr_period;
                            document.getElementById('atrValue').innerText = data.current_settings.atr_period;
                        }
                        if (parseFloat(data.current_settings.stop_loss_percent * 100) !== parseFloat(document.getElementById('stop_loss').value)) {
                            document.getElementById('stop_loss').value = data.current_settings.stop_loss_percent * 100;
                            document.getElementById('stopLossValue').innerText = (data.current_settings.stop_loss_percent * 100).toFixed(1) + '%';
                        }
                        if (parseFloat(data.current_settings.min_profit_usd) !== parseFloat(document.getElementById('min_profit_usd').value)) {
                            document.getElementById('min_profit_usd').value = data.current_settings.min_profit_usd;
                        }
                        if (data.current_settings.heikin_ashi.toString() !== document.getElementById('heikin_ashi').value) {
                            document.getElementById('heikin_ashi').value = data.current_settings.heikin_ashi;
                        }
                    }
                    location.reload();
                });
...
class CryptoTradingBot:
    """Multi-symbol UT Bot for Crypto trading with Live Web Controls"""
    ...
    def _log_configuration(self):
        ...
        logger.info("="*50)
        logger.info("UT Bot Multi-Crypto Started (Live Controls)")
        logger.info("="*50)
        ...
        logger.info("   You can change settings LIVE from the dashboard!")
        logger.info("="*50)
    ...
    def update_setting(self, setting, value):
        ...
        if setting == 'timeframe':
            ...
            self._update_timeframe_settings()
            self.add_log(f"⚙️ Timeframe changed to {value} (check interval: {self.check_interval}s)")
        elif setting == 'sensitivity':
            self.a = float(value)
            self.add_log(f"⚙️ Sensitivity (a) changed to {self.a}")
    ...
    def _update_env_file(self, setting, value):
        ...
        if setting in env_key_map:
            ...
            try:
                ...
                self.add_log(f"Warning: Could not update .env file: {e}", "ERROR")
    ...
    def run_strategy(self):
        ...
        self.add_log(f"🚀 Starting Multi-Crypto UT Bot with Live Controls")
        ...
        self.add_log(f"🌐 Change settings live at http://localhost:5000")
...
def main():
    print("\n" + "="*50)
    print("🤖 UT Bot Multi-Crypto - Live Web Controls")
    print("="*50)
    ...
    try:
        ...
        print("   You can change settings LIVE from the dashboard!")
        print("   No restart needed for:")
        print("   - Timeframe")
        print("   - Sensitivity (a)")
        print("   - ATR Period")  
        print("   - Stop Loss %")
        ...
        print("   - Min Profit $")
        print("")
        response = input("🚀 Start paper trading? (yes/no): ").lower().strip()
        if response == 'y':
