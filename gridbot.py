...
<body>
    <div class="container">
        ...
        <div class="grid">
            ...
            <div class="card">
                <h3>📈 Active Positions (Net P&L after fees)</h3>
        ...
        <div class="grid">
            ...
            <div class="card">
                ...
                <div class="info-note">📊 All P&L shown are NET (after spread + fees)</div>
            </div>
        </div>
    </div>
...
class AlpacaSpotBot:
    ...
    def is_profitable_enough(self, symbol, entry_price, current_price, qty):
        ...
        if net_pnl < self.min_net_profit:
            self.add_log(f"⚠️ {symbol} Net profit ${net_pnl:.4f} < ${self.min_net_profit:.2f} - holding")
            return False

        self.add_log(f"✅ {symbol} Net profit ${net_pnl:.4f} >= ${self.min_net_profit:.2f} - proceed")
        return True
            ...
            if buy_signal and not has_position:
                ...
                if position_info:
                    ...
                    if self.grid_stops.get(symbol):
                        self.add_log(f"🔴 {display} - Grid stop active. Following grid stop.")
                    else:
                        if self.is_profitable_enough(display, entry_price, latest_close, qty):
                            self.add_log(f"🔴 {display} - Closing position (profitable)")
                            self.close_position(symbol)
                        else:
                            self.add_log(f"🔴 {display} - Holding (net profit below threshold)")
                else:
                    self.close_position(symbol)
    ...
    def run_strategy(self):
        ...
        self.add_log(f"🚀 Starting ALPACA SPOT BOT - Net P&L Tracking")
...
def main():
    print("\n" + "="*50)
    print("🤖 ALPACA SPOT BOT - NET P&L VERSION")
    print("="*50)
    ...
    try:
        ...
        print("   📊 All P&L shown are NET after spread + fees")
        print("   🛡️ Spread protection active")
        print("   🎯 Manual Buy/Close buttons available")
        print("")
        ...
        if response == 'y':
            ...
            print("📊 EXIT: Grid stop (never sell at a loss)")
            print("💰 All P&L displayed is NET after costs")
            bot.run_strategy()
        else:
            print("Bot stopped.")
    except Exception as e:
