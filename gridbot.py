...
<html>
<head>
    <title>ALPACA SPOT BOT - UT Bot Entry + Grid Stop Exit</title>
...
<body>
    <div class="container">
        ...
        <div style="text-align: center; margin-bottom: 20px;">
            <span class="status-badge" style="background:#00ff88; color:#000;">UT Bot Entry</span>
            <span class="status-badge" style="background:#ffaa00; color:#000;">Grid Stop Exit</span>
            <span class="status-badge" style="background:#00d4ff; color:#000;">Net P&L</span>
        </div>
        ...
        <div class="grid">
            ...
            <div class="card">
                <h3>📈 Active Positions (Net P&L after fees)
        ...
        <div class="grid">
            ...
            <div class="card">
                ...
                <table class="data-table">
                    <thead><tr><th>Symbol</th><th>Price</th><th>Trailing Stop</th><th>UT Signal</th><th>Position</th><th>Actions</th></tr></thead>
            ...
            <div class="card">
                ...
                <div class="info-note">🎯 BUY: UT Bot signal | 📊 EXIT: Grid stop (guaranteed profit)</div>
            </div>
        </div>
    </div>
        ...
        function updateDashboard(data) {
            ...
            if (data.positions && data.positions.length > 0) {
                ...
                for (let pos of data.positions) {
                    ...
                    html += `<tr><td>${pos.symbol}</td><td>${pos.qty.toFixed(6)}</td><td>$${pos.entry_price.toFixed(2)}</td><td>$${pos.current_price.toFixed(2)}</td><td class="${pnlClass}">$${pos.net_pnl.toFixed(2)}</td><td class="positive">${pos.grid_stop ? '$'+pos.grid_stop.toFixed(2) : 'N/A'}</td><td class="action-buttons"><button onclick="manualAction('${pos.symbol}','close')" class="danger">Close</button></td></tr>`;
                }
            ...
            if (data.symbols && data.symbols.length > 0) {
                ...
                for (let sym of data.symbols) {
                    let signalBadge = sym.signal_type ? `<span class="status-badge signal-buy">${sym.signal_type}</span>` : '<span class="status-badge" style="background:#888">NONE</span>';
                    ...
                    html += `<tr><td>${sym.symbol}</td><td>$${sym.price.toFixed(2)}</td><td>$${sym.trailing_stop.toFixed(2)}</td><td>${signalBadge}</td><td>${positionBadge}</td><td class="action-buttons"><button onclick="manualAction('${sym.symbol}','buy')" class="success">Buy</button><button onclick="manualAction('${sym.symbol}','close')" class="danger">Close</button></td></tr>`;
                }
...
class AlpacaSpotBot:
    """Alpaca Spot Bot - UT Bot Entry + Grid Stop Exit with Net P&L"""
    ...
    def _update_timeframe_settings(self):
        ...
        self.interval_map = {"1Min": 60, "5Min": 300, "15Min": 900, "30Min": 1800, "1H": 3600, "2H": 7200, "4H": 14400, "1D": 86400}
    ...
    def _log_configuration(self):
        ...
        logger.info("="*50)
        logger.info("ALPACA SPOT BOT - UT Bot Entry + Grid Stop Exit")
        logger.info("="*50)
        ...
        logger.info(f"Spread: {self.spread_percent*100:.1f}% | Fees: {self.fee_percent*100:.1f}%")
        logger.info(f"Min Net Profit: ${self.min_net_profit}")
        logger.info("STRATEGY: UT Bot BUY signals | Grid Stop EXIT (guaranteed profit)")
        logger.info("="*50)
    ...
    def _update_env_file(self, setting, value):
        ...
        env_key_map = {'timeframe': 'TIMEFRAME', 'sensitivity': 'SENSITIVITY', 'atr_period': 'ATR_PERIOD', 'grid_size': 'GRID_SIZE', 'min_net_profit': 'MIN_NET_PROFIT', 'spread_percent': 'SPREAD_PERCENT', 'heikin_ashi': 'HEIKIN_ASHI'}
    ...
    def get_current_settings(self):
        return {'timeframe': self.timeframe_str, 'sensitivity': self.a, 'atr_period': self.atr_period, 'grid_size': self.grid_size, 'min_net_profit': self.min_net_profit, 'spread_percent': self.spread_percent, 'heikin_ashi': self.use_heikin_ashi}

    def _parse_timeframe_to_minutes(self, timeframe_str):
        timeframe_map = {"1Min": 1, "5Min": 5, "15Min": 15, "30Min": 30, "1H": 60, "2H": 120, "4H": 240, "1D": 1440}
    ...
    def calculate_grid_stop(self, entry_price, current_price, qty):
        position_value = entry_price * qty
        spread_cost = position_value * self.spread_percent
        fee_cost = position_value * self.fee_percent * 2
        total_costs = spread_cost + fee_cost
    ...
    def calculate_signals(self, df):
        """UT Bot - BUY signals only (SELL ignored, grid stop used for exit)"""
        df = df.copy()
    ...
    def get_historical_data(self, symbol, bars=None):
        ...
        request = CryptoBarsRequest(symbol_or_symbols=symbol, timeframe=timeframe, start=start, end=end, limit=bars + 150)
        ...
        if df.empty or len(df) < 100:
            return None
        df = df.reset_index()
        df = df.rename(columns={'timestamp': 'datetime', 'open': 'open', 'high': 'high', 'low': 'low', 'close': 'close'})
        ...
        try:
            ...
            for pos in positions:
                if pos.symbol == alpaca_symbol:
                    ...
                    self.add_log(f"✅ Closed {symbol}: {qty} @ ${fill_price:.2f} | Gross: ${gross_pnl:.2f} | Costs: ${costs:.2f} | NET: ${net_pnl:.2f}")
        ...
        try:
            ...
            for pos in all_positions:
                ...
                if display_symbol in self.symbols:
                    ...
                    current = float(pos.current_price)
                    qty = float(pos.qty)
                    net_pnl = self.get_position_net_pnl(entry, current, abs(qty))
                    grid_stop, _ = self.calculate_grid_stop(entry, current, abs(qty))
                    positions.append({
    ...
    def process_symbol(self, symbol):
        try:
            ...
            # EXIT: Grid stop for open positions
            if has_position and position_info:
                entry_price = position_info['entry']
                qty = position_info['qty']
                grid_stop, is_active = self.calculate_grid_stop(entry_price, latest_close, qty)
            ...
            if symbol not in self._last_log_time or (current_time - self._last_log_time[symbol]) > 30:
                self.add_log(f"{display}: ${latest_close:.2f} | Stop: ${latest_trailing_stop:.2f} | Signal: {1 if buy_signal else 0}")
                self._last_log_time[symbol] = current_time

            # ENTRY: UT Bot BUY signal only
            if buy_signal and not has_position:
                self.add_log(f"🎯 {display} 🔵 UT Bot BUY SIGNAL at ${latest_close:.2f}")
                self.add_log(f"🔵 {display} - Going LONG (Grid stop will manage exit)")
    ...
    def run_strategy(self):
        global bot_instance
        bot_instance = self
        self.add_log(f"🚀 Starting ALPACA SPOT BOT - UT Bot Entry + Grid Stop Exit")
        ...
        self.add_log(f"🛡️ All P&L shown are NET after {self.spread_percent*100:.1f}% spread + {self.fee_percent*100:.1f}% fees")
        ...
        for symbol in self.symbols:
            try:
                ...
                if df is not None:
                    ...
                    signal_type = 'BUY' if latest['buy_signal'] and not has_position else None
                    symbols_data.append({
                ...
                symbols_data.append({'symbol': symbol.replace('/USD', ''), 'price': 0, 'trailing_stop': 0, 'signal_type': None, 'position_status': 'unknown'})
...
def main():
    print("\n" + "="*50)
    print("🤖 ALPACA SPOT BOT - UT Bot Entry + Grid Stop Exit")
    print("="*50)
    print("  ENTRY: UT Bot BUY signal (price crosses above ATR trailing stop)")
    print("  EXIT:  Grid stop (guaranteed profit after spread/fees)")
    ...
    try:
        ...
        print("   📊 UT Bot signals for entry only")
        print("   🎯 Grid stop for exit (guaranteed profit)")
        ...
        if response == 'y':
            ...
            print("📈 BUY: UT Bot signal | EXIT: Grid stop (never sell at loss)")
            bot.run_strategy()
        else:
            print("Bot stopped.")
    except Exception as e:
