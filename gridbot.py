...
# HTML Template for Dashboard with Input Controls
DASHBOARD_TEMPLATE = """
<!DOCTYPE html>
<html>
<head>
...
<body>
    <div class="container">
        ...
        <div class="grid">
            ...
            <div class="card">
                <h3>📈 Active Positions <button onclick="fetch('/close_all_positions',{method:'POST'})" class="danger" style="float:right;">Close All</button></h3>
                <table class="data-table">
                    <thead><tr><th>Symbol</th><th>Qty</th><th>Entry</th><th>Current</th><th>P&L</th><th>Grid Stop</th><th>Action</th></tr></thead>
    ...
    <script>
        ...
        function updateDashboard(data) {
            ...
            if (data.account) {
                ...
                let pnlClass = data.account.total_pnl >= 0 ? 'positive' : 'negative';
                document.getElementById('todayPnl').innerHTML = '<span class="' + pnlClass + '">$' + (data.account.today_pnl?.toFixed(2) || '0.00') + ' (' + (data.account.today_pnl_percent?.toFixed(2) || '0.00') + '%)</span>';
                document.getElementById('realizedPnl').innerHTML = '<span class="' + pnlClass + '">$' + (data.account.total_realized_pnl?.toFixed(2) || '0.00') + '</span>';
                document.getElementById('unrealizedPnl').innerHTML = '<span class="' + pnlClass + '">$' + (data.account.unrealized_pnl?.toFixed(2) || '0.00') + '</span>';
                document.getElementById('totalPnl').innerHTML = '<span class="' + pnlClass + '">$' + (data.account.total_pnl?.toFixed(2) || '0.00') + ' (' + (data.account.total_pnl_percent?.toFixed(2) || '0.00') + '%)</span>';
            }
...
"""
class AlpacaSpotBot:
    """Alpaca Spot Bot - Grid Trailing Stop with Guaranteed Profit"""
    ...
    def _load_settings_from_env(self):
        ...
        # Grid stop settings
        self.grid_size = float(os.getenv('GRID_SIZE', '0.001'))  # 0.1% default
        self.first_profit_level = float(os.getenv('FIRST_PROFIT_LEVEL', '0.009'))  # 0.9% before first stop
    ...
    def _log_configuration(self):
        ...
        logger.info("-"*30)
        logger.info("GRID STOP STRATEGY:")
        logger.info(f"  First stop activates at +{self.first_profit_level*100:.1f}% after fees")
        logger.info(f"  New stop every +{self.grid_size*100:.2f}%")
        logger.info(f"  Exit when price falls 0.02% below last stop")
        logger.info("-"*30)
    ...
    def update_setting(self, setting, value):
        ...
        if setting == 'timeframe':
            ...
            self.add_log(f"⚙️ Grid Step changed to {self.grid_size*100:.2f}%")
    ...
    def calculate_grid_stop(self, symbol, entry_price, current_price, qty):
        ...
        Logic:
        - First profitable level: entry + 0.9% (after spread/fees)
        - Then every +Grid Step creates a new stop level
        - Exit if price falls 0.02% below last locked level
        """

        # Calculate costs (spread + fees)
        ...
        if current_price >= first_profit_level_price:
            ...
            self.add_log(f"📊 {symbol} Grid Stop: Level {levels_above} | Stop: ${current_stop:.2f} | Net Profit: {net_profit_percent:.2f}%")
    ...
    def close_all_positions(self):
        try:
            positions = self.trading_client.get_all_positions()
            for pos in positions:
                display_symbol = self.normalize_symbol(pos.symbol, to_display=True)                                                                                     if display_symbol in self.symbols:
                    self.close_position(display_symbol)                                 self.add_log("✅ Closed all positions")
            return True
        except Exception as e:                                                          self.add_log(f"Error closing all positions: {e}", "ERROR")
            return False                                                                                                                                    def get_account_status(self):                                                   try:
            account = self.trading_client.get_account()                                 current_equity = float(account.equity)
            ...
            try:
                ...
                for pos in positions:
                    if self.normalize_symbol(pos.symbol, to_display=True) in self.symbols:
                        unrealized_pnl += float(pos.unrealized_pl)                      except Exception:
                pass

            realized_pnl = total_pnl - unrealized_pnl

            current_date = datetime.now().date()
            if self.day_start_equity is None:                                               self.day_start_equity = current_equity
            ...
            return {                                                                        'buying_power': float(account.buying_power),
                ...
                'today_pnl': today_pnl,                                                     'total_realized_pnl': realized_pnl,
                'unrealized_pnl': unrealized_pnl,                                           'total_pnl': total_pnl,
                'today_pnl_percent': today_pnl_percent,                                     'total_pnl_percent': total_pnl_percent,
                'starting_balance': self.starting_balance
            }
        except Exception as e:
            return {
                'buying_power': 0, 'portfolio_value': 0,                                    'today_pnl': 0, 'total_realized_pnl': 0,
    ...
    def get_all_positions(self):
        positions = []
        try:                                                                            all_positions = self.trading_client.get_all_positions()
            for pos in all_positions:                                                       display_symbol = self.normalize_symbol(pos.symbol, to_display=True)                                                                                     if display_symbol in self.symbols:
                    entry = float(pos.avg_entry_price)                                          grid_stop, _ = self.calculate_grid_stop(display_symbol, entry, float(pos.current_price), abs(float(pos.qty)))                                           positions.append({
                        'symbol': display_symbol,                                                   'qty': float(pos.qty),
                        'entry_price': entry,                                                       'current_price': float(pos.current_price),
    ...
    def process_symbol(self, symbol):
        try:
            ...
            if has_position and position_info:                                              entry_price = position_info['entry']
                ...
                self.grid_stops[symbol] = grid_stop
                                                                                            # Check if price hit grid stop
                if is_active and latest_close <= grid_stop:
                    self.add_log(f"🎯 {display} Grid Stop Hit! Selling at ${latest_close:.2f}")
            ...
            if symbol not in self._last_log_time or (current_time - self._last_log_time[symbol]) > 30:
                self.add_log(f"{display}: ${latest_close:.2f} | Stop: ${latest_trailing_stop:.2f}")                                                                     self._last_log_time[symbol] = current_time
                                                                                        # BUY SIGNAL
            if buy_signal and not has_position:                                             self.add_log(f"🎯 {display} 🔵 BUY SIGNAL at ${latest_close:.2f}")
                ...
                if position_info:
                    ...
                    if self.grid_stops.get(symbol):
                        self.add_log(f"🔴 {display} - Grid stop active at ${self.grid_stops[symbol]:.2f}. Following grid stop.")
                    else:                                                                           position_value = entry_price * qty
                        spread_cost = position_value * self.spread_percent                          fee_cost = position_value * self.fee_percent * 2
                        total_costs = spread_cost + fee_cost                
                        pnl = (latest_close - entry_price) * qty                                    net_profit = pnl - total_costs
                                                                                                    if net_profit >= self.min_net_profit:
                            self.add_log(f"🔴 {display} - Closing position (profitable)")
                            self.close_position(symbol)                                             else:
                            self.add_log(f"🔴 {display} - Holding (net profit ${net_profit:.4f} < ${self.min_net_profit:.2f})")
                else:                                                                           self.close_position(symbol)
                                                                                    except Exception as e:
            self.add_log(f"Error processing {symbol}: {e}", "ERROR")        
    def run_strategy(self):                                                         global bot_instance
        bot_instance = self                                                 
        self.add_log(f"🚀 Starting ALPACA SPOT BOT - Grid Trailing Stop")           self.add_log(f"💰 Starting Balance: ${self.starting_balance:,.2f}")
        self.add_log(f"📊 UT Bot: Price crosses ATR trailing stop (a={self.a}, ATR={self.atr_period})")
        self.add_log(f"📊 Grid Stop: First at +{self.first_profit_level*100:.1f}%, then every +{self.grid_size*100:.2f}%")
        self.add_log(f"🛡️ Spread Protection: {self.spread_percent*100:.1f}% spread, ${self.min_net_profit} min net profit")
        self.add_log(f"📊 Monitoring: {', '.join(self.symbols)}")                   self.add_log(f"🌐 Web dashboard: http://localhost:5000")
                                                                                    while self.running:
            for symbol in self.symbols:                                                     if not self.running:
                    break                                                                   self.process_symbol(symbol)
                time.sleep(2)                                                           time.sleep(self.check_interval)
                                                                                def get_dashboard_data(self):
        account = self.get_account_status()                                         positions = self.get_all_positions()
                                                                                    symbols_data = []
        for symbol in self.symbols:                                                     try:
                df = self.get_historical_data(symbol, bars=150)                             if df is not None:
                    signals = self.calculate_signals(df)                                        has_position = self.get_position_status(symbol)
                    latest = signals.iloc[-1]                               
                    signal_type = None                                                          if latest['buy_signal'] and not has_position:
                        signal_type = 'BUY'                                                     elif latest['sell_signal'] and has_position:
                        signal_type = 'SELL'                                
                    symbols_data.append({                                                           'symbol': symbol.replace('/USD', '').replace('USDT', ''),                                                                                               'price': float(latest['close']),
                        'trailing_stop': float(latest['trailing_stop']),                            'signal_type': signal_type,
                        'position_status': 'long' if has_position else 'flat'
                    })                                                                  except Exception:
                symbols_data.append({                                                           'symbol': symbol.replace('/USD', '').replace('USDT', ''),                                                                                               'price': 0, 'trailing_stop': 0,
...
def api_status():                                                               global bot_instance
    if not bot_instance:                                                            return jsonify({
            'logs': ['Bot not running'],                                                'current_settings': {},
            'bot_start_time': 0,                                                        'bot_running': False
        })                                                                  
    account, positions, symbols_data = bot_instance.get_dashboard_data()        current_settings = bot_instance.get_current_settings()
                                                                                return jsonify({
        'account': account,                                                         'positions': positions,
        'symbols': symbols_data,                                                    'logs': bot_instance.logs[-50:],
        'current_settings': current_settings,                                       'bot_start_time': int(bot_instance.bot_start_time),
        'bot_running': bot_instance.running                                     })
                                                                            
@app.route('/update_setting', methods=['POST'])                             def update_setting():
    global bot_instance                                                         if not bot_instance:
        return jsonify({'status': 'error', 'message': 'Bot not running'})   
    data = request.get_json()                                                   setting = data.get('setting')
    value = data.get('value')                                               
    if bot_instance.update_setting(setting, value):                                 return jsonify({'status': 'ok', 'message': f'{setting} updated to {value}'})                                                                        else:
        return jsonify({'status': 'error', 'message': 'Failed to update setting'})
                                                                            
@app.route('/manual_action', methods=['POST'])                              def manual_action():
    global bot_instance                                                         if not bot_instance:
        return jsonify({'status': 'error', 'message': 'Bot not running'})   
    data = request.get_json()                                                   symbol = data.get('symbol')
    action = data.get('action')                                             
    alpaca_symbol = f"{symbol}/USD"                                         
    if action == 'buy':                                                             result = bot_instance.manual_buy(alpaca_symbol)
    elif action == 'close':                                                         result = bot_instance.manual_sell(alpaca_symbol)
    else:                                                                           return jsonify({'status': 'error', 'message': 'Invalid action'})
                                                                                if result:
        return jsonify({'status': 'ok', 'message': f'{action} executed for {symbol}'})
    else:                                                                           return jsonify({'status': 'error', 'message': f'Failed to execute {action} for {symbol}'})                                                      @app.route('/close_all_positions', methods=['POST'])
def close_all_route():                                                          global bot_instance
    if bot_instance:                                                                bot_instance.close_all_positions()
    return jsonify({'status': 'ok'})                                        
                                                                            @app.route('/stop', methods=['POST'])
def stop_route():                                                               global bot_instance
    if bot_instance:                                                                bot_instance.running = False
    return jsonify({'status': 'stopping'})                                  
                                                                            def run_dashboard():
    app.run(host='0.0.0.0', port=5000, debug=False, use_reloader=False)     
                                                                            def main():
    print("\n" + "="*50)                                                        print("🤖 ALPACA SPOT BOT - GRID TRAILING STOP")
    print("="*50)                                                               print("  BUY:  Price crosses ABOVE ATR trailing stop")
    print("  SELL: Grid stop when price falls below locked profit level")       print("  GRID: First stop at +0.9%, then every +0.1%")
    print("="*50)                                                               print("Loading configuration from .env file...")
